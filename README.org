...
- You have support for two different module systems (/CommonJS/ and /ES6 modules/).
...
*** Importing modules (CommonJS vs ES6 modules)
...
In the above example we are using the default CommonJS style imports (~require~). In the examples in this course we are mainly going to use ES6 modules (~import~, ~export~), also referred to as EcmaScript modules or JavaScript modules. You are free to use whichever style you are most comfortable with. If you want to use the ~import~ syntax you need to edit =package.json= and set the ~type~ property to ~module~ as in the example below. You can generate a =package.json= file by running ~npm init~ in the terminal and just accept all of the suggested default settings.
...
*** TODO Accessing environment variables
* Our First API
